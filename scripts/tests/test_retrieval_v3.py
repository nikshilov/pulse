...
def seed_events(con):
    ...
    def fake_embedding(text: str) -> tuple[int, str]:
        vec = embed_texts([text], model="fake-local")[0]
        return len(vec), json.dumps(vec)

    anya_dim, anya_vec = fake_embedding("что у меня с Аней?")
    tired_dim, tired_vec = fake_embedding("я устал")
    chain_dim, chain_vec = fake_embedding("zasluzhivatel shame wound")

    # Embeddings generated by the same fake-local embedder used at query time.
    embs = [
        (1, "fake-local", anya_dim, anya_vec, "что у меня с Аней?"),
        (2, "fake-local", tired_dim, tired_vec, "я устал"),
        (22, "fake-local", chain_dim, chain_vec, "zasluzhivatel shame wound"),
        (33, "fake-local", chain_dim, chain_vec, "zasluzhivatel shame wound"),
    ]
    con.executemany(
...
def test_date_proximity_boost_when_snapshot_date_set(seed_events):
    """Phase 5.2: state.snapshot_days_ago activates date-proximity boost.

    Events close to the snapshot date get a small score boost; formula
    collapses to identical ordering when snapshot_days_ago is None.
    """
    con = seed_events
    # Without snapshot date → no date boost
    baseline = retrieve_events_v3(con, "zasluzhivatel shame wound", top_k=3,
                                    embedder_model="fake-local")
    # With snapshot_days_ago close to event 33's ts (2026-04-17 → days_ago ≈ 5)
    state = UserState(snapshot_days_ago=5.0)
    with_date = retrieve_events_v3(con, "zasluzhivatel shame wound", top_k=3,
                                    embedder_model="fake-local", user_state=state)
    # At least one event should show date_boost key when snapshot set
    assert any("date_boost" in ev for ev in with_date), \
        f"Expected date_boost on at least one event, got {with_date}"
    # No date boost key when snapshot_days_ago is None
    assert not any("date_boost" in ev for ev in baseline), \
        f"Did not expect date_boost without snapshot, got {baseline}"


def test_date_proximity_off_when_no_snapshot(seed_events):
    """Phase 5.2: user_state WITHOUT snapshot_days_ago → no date boost → identical
    to baseline (v2_pure behavior preserved for regular stateful queries)."""
    con = seed_events
    baseline = retrieve_events_v3(con, "что у меня с Аней?", top_k=3,
                                    embedder_model="fake-local")
    # State with mood but no date → no date boost
    state_no_date = UserState(mood_vector={"shame": 0.8}, snapshot_days_ago=None)
    with_mood_no_date = retrieve_events_v3(con, "что у меня с Аней?", top_k=3,
                                            embedder_model="fake-local",
                                            user_state=state_no_date)
    for ev in with_mood_no_date:
        assert "date_boost" not in ev, f"No date boost expected, got {ev}"
